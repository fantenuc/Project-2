...
def find_urls(s):
    url = re.findall('http[s]?://\S*\.\S{2,}', s) # tests for http:// and https://, at least 1 ., and at least 2 characters after each .
    # print(url)
    return url
...
def grab_headlines():
    ## For use with the opinion.html file when testing
    # f = open('opinion.html', 'r')
    # html_file = f.read()
    # soup = BeautifulSoup(html_file, 'html.parser')

    ## For use when running the code to pull from the Michigan Daily website
    url = 'http://www.michigandaily.com/section/opinion'
    html = requests.get(url)  
    soup = BeautifulSoup(html.content, 'html.parser') # takes the url content and parses it into a html using BeautifulSoup  

    # print(soup.prettify())
    
    tags = soup('ol') # finds the headings of the Most Read section of Michigan Daily
    for header in tags:
        headers = header.text.strip().split('\n') # converts to text and splits at new line character
    return headers
...
def get_umsi_data():
    umsi_titles = {} # return dictionary
    start_url = 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All' # starting url (page 1 of 12)
    page_num = 0
    
    while page_num < 13:
        updated_url = start_url + '&page=' + str(page_num) # updates url from page 1 to page 12
        html = requests.get(updated_url, headers = {'User-Agent': 'SI_CLASS'})
        soup = BeautifulSoup(html.content, 'html.parser') # parses url into html using BeautifulSoup
        
        # print (soup.prettify())
        
        names = soup.find_all('div', class_ = 'field-item even', property = 'dc:title') # finds names of faculty on page
        titles = soup.find_all('div', class_ = 'field field-name-field-person-titles field-type-text field-label-hidden') # finds titles of faculty on page
        
        title_to_name = 0
        for name in names:
            umsi_titles[name.text] = titles[title_to_name].text # adding names and titles to return dictionary
            title_to_name += 1
        
        page_num += 1 # to go to next page
...
def num_students(data):
    ...
    for info in data.values():
        if info == 'PhD student':
            PhD_count += 1 # adding faculty to count if they are a PhD student
    return PhD_count
